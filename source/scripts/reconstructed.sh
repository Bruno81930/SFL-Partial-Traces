...
function reconstructed() {
  ...
  for PERCENTAGE in $(seq 0 10 100); do
    ...
    COMMAND="java -Xmx25G -jar logs.jar reconstructed -p $PROJECT -b $BUG -P $PERCENTAGE -d data -s $SOURCE -s $TEST -f RANDOM_WITH_FAULTS"
...
if [ "$ARG_PROJECT" = "all" ]; then
    ...
    if [ "$ARG_PERCENTAGE" != "" ]; then
      single_reconstructed "$ARG_PROJECT" "$ARG_VERSION" "src" "test" "$ARG_PERCENTAGE"
