...
travis_cmd cd\ failed/Adobe-Consulting-Services/acs-aem-commons --assert --echo
...
export TRAVIS_BUILD_DIR=$HOME/build/failed/Adobe-Consulting-Services/acs-aem-commons
...
export TRAVIS_REPO_SLUG=Adobe-Consulting-Services/acs-aem-commons
...
travis_cmd export\ MAVEN_SKIP_RC\=true --echo
travis_cmd export\ MAVEN_OPTS\=\"-Xmx512m\ -XX:MaxPermSize\=512m\ -Duser.language\=de\ -Duser.country\=DE\" --echo
travis_cmd export\ POST_COVERAGE\=false --echo
echo
...
travis_fold start install
travis_cmd mvn\ -o\ install\ -Dhttps.protocols=TLSv1.2\ -DskipTests\=true\ -Dmaven.javadoc.skip\=true\ -B\ -V --assert --echo --retry --timing
travis_fold end install
fi

travis_cmd mvn\ verify\ -Dhttps.protocols=TLSv1.2\ -B --echo --timing
travis_result $?

if [[ $TRAVIS_TEST_RESULT = 0 ]]; then
travis_fold start after_success
travis_cmd if\ \[\ \"\$POST_COVERAGE\"\ \=\=\ \"true\"\ \]\;\ then\ mvn\ clean\ jacoco:prepare-agent\ test\ jacoco:report\ coveralls:report\;\ fi\ -Dhttps.protocols=TLSv1.2 --echo --timing
